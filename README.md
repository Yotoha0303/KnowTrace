...
- 调查中的未审核 Evidence 可以编辑；每次编辑保留旧版本，并把当前来源检查重置为待检查。来源 URL 可以留空，但无链接 Evidence 当前不能检查或采纳。
- 每条 Evidence 可附加最多 5 张 JPEG、PNG、WebP 或 GIF 图片（单张不超过 10 MB），文件保存在项目 `data/uploads/evidence`，数据库保存相对路径、格式、大小和 SHA-256；图片只是附件，不代表已经核验。
