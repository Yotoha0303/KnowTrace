...
- KnowTrace 自建另一套密码存储和账号数据库；认证统一由仓库内 `services/go-user-system` 的独立服务负责。当前已按创建者隔离内容，但尚未实现 Workspace/组织分组。
...
KnowTrace 业务数据按创建者隔离：拥有 `admin` 角色的管理员可读取和管理全部用户创建的记录、分类及其派生内容；普通成员只可读取和管理自己的内容，直接访问其他成员的页面、API、搜索结果、导出文件或证据图片都会返回不可见结果。历史迁移数据标记为 `legacy-local`，仅管理员可见。
...
第一版 Web 应用已实现：快速录入、记录编辑与删除、乐观版本控制、修改历史、多分类、分类管理、归档恢复、AI 结构化整理、处理状态反馈、分类数量约束、可选局部原文建议、来源片段约束、人工接受/修改/驳回，以及 AI 处理历史。P1.1–P1.4 已形成“候选主张—来源检查—证据审核—人工结论—AI 非裁决审查”闭环；P2 已加入统一知识检索、Category 主题档案、对象/时间筛选和可解释的相似记录；P2.6 已完整接入并内置 go-user-system 账号与 RBAC 后端，并实现管理员全局、成员仅本人的资源级数据隔离；P2.7 已加入对象时间线及可追溯的 AI 主题综合档案；P2.8 已加入来源权威性评估、身份化独立复核和不可变可靠知识发布版本；P2.9 已交付未来 App 可复用的 `/api/v1` 读取与 Capture 生命周期接口。尚未实现自动联网补证、Workspace/组织分组、更细粒度的业务角色、移动 App 和公网发布通道，界面仍不存在含糊的“已验证”入口。
