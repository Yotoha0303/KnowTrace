...
### 下一迭代前置项：CC-Switch 当前供应商自动识别

状态：已记录，暂缓实现。下一次迭代先完成 `US-15`：选择 CC-Switch 后自动识别并显示 Codex、DeepSeek 或其他实际供应商，并在外部切换后重新检测。当前版本只完成代理健康检查与模型能力测试，不能把通用的 `ccswitch-current-provider` 运行标识声称为已识别的真实供应商。

同时已把四项使用中发现的问题登记到 [`14-deferred-issues.md`](./14-deferred-issues.md)：无变化保存仍增加 Revision、AI 操作前重复要求连接测试、管理员创建内容的成员可见性，以及同一用户重复导入同一记录。当前只记录，不在本轮实现；进入下一次开发时先复现并建立失败测试。

完成该前置项后，下一阶段再从 Workspace 数据隔离、细粒度角色授权、真正的移动 App、自动联网补证中选择一个，不同时展开。
