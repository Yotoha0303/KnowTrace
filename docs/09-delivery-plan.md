...
- 当前允许 Evidence 不填写来源 URL，并继续保存文字、备注和图片附件。
- 当前图片只作为附件保存；无链接 Evidence 不能自动检查、采纳或推动 Claim 提交待审核。
- 暂不通过解除按钮限制来绕过来源门槛，也不把“已上传”视为“已核验”。
- 后续重新启动该需求时，先根据真实样本确定核验对象与步骤，再设计不可变核验记录、Evidence 版本和附件哈希绑定、失效规则，以及个人/小群体审核者身份。
- 重新启动条件：已经积累足够的截图、聊天记录、现场照片或线下材料样本，且这些样本因缺少公开 URL 明显阻碍日常证据整理。
