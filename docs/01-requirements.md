...
- 来源 URL 可以留空；Evidence 仍可保存、编辑、上传图片或人工排除。
- 图片目前只是附件，不代表已经核验，也不能替代网页来源检查。
- 无来源 URL 的 Evidence 不能自动检查、采纳或用于推动 Claim 提交待审核。
- 本需求暂缓，不在当前阶段改变现有状态机或降低来源确认门槛。

未来验收条件候选：

- 人工核验必须显式记录结果、依据、时间、核验范围和所使用的附件。
- 核验记录必须绑定 Evidence 版本与附件 SHA-256；Evidence 或附件变化后旧核验不得继续生效。
- “已保存”“已核验”“已采纳”必须是不同状态，上传图片不能自动推进状态。
- 网页自动检查与图片/线下材料人工核验应作为不同核验路径，不能互相伪装。
- 启用小群体共享审核前，需要先决定审核者身份、权限和职责分离方式。
