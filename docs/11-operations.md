...
首次启动会在 `.env` 生成 MySQL/JWT 随机密钥，并在尚无管理员时创建固定默认管理员 `KnowTrace / KnowTrace@123`。它类似常见中间件的默认凭据，只适合绑定 `127.0.0.1` 的本机首次登录。重复启动只检查管理员是否已存在，不会把用户后来修改的密码重置回默认值；修改密码后，MySQL 中的 bcrypt 哈希是当前凭据，`.env` 中的值仍只是初始化默认值。需要独立复核时仍须准备另一个不同账号；结论作者与复核者不能共享账号。修改密码会使该账号的全部会话失效；当前认证后端不提供设备会话列表和单设备撤销接口。管理员角色分配依赖数字用户 ID，因为后端尚无用户列表接口。
